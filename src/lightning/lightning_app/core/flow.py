...
from lightning.lightning_app.storage import Path
from lightning.lightning_app.storage.drive import _maybe_create_drive, Drive
from lightning.lightning_app.utilities.app_helpers import _is_json_serializable, _LightningAppRef, _set_child_name, is_overridden
from lightning.lightning_app.utilities.component import _sanitize_state
from lightning.lightning_app.utilities.exceptions import ExitAppException
from lightning.lightning_app.utilities.introspection import _is_init_context, _is_run_context
from lightning.lightning_app.utilities.packaging.cloud_compute import _maybe_create_cloud_compute, CloudCompute
...
class LightningFlow:
    ...
    def __init__(self):
        """The LightningFlow is used by the :class:`~lightning.lightning_app.core.app.LightningApp` to coordinate and manage
        long- running jobs contained, the :class:`~lightning.lightning_app.core.work.LightningWork`.

        A LightningFlow is characterized by:

        * A set of state variables.
        * Long-running jobs (:class:`~lightning.lightning_app.core.work.LightningWork`).
        ...
        **Execution model and work**
        ...
        The actions themselves are self-contained within :class:`~lightning.lightning_app.core.work.LightningWork`.
        The :class:`~lightning.lightning_app.core.work.LightningWork` are typically used for long-running jobs,
        ...
        **Component hierarchy and App**
        ...
        Example:
            ...
            >>> from lightning.lightning_app import LightningFlow
    ...
    def __setattr__(self, name: str, value: Any) -> None:
        ...
        from lightning.lightning_app.structures import Dict, List
    ...
    def works(self, recurse: bool = True) -> List[LightningWork]:
        """Return its :class:`~lightning.lightning_app.core.work.LightningWork`."""
    ...
    def named_works(self, recurse: bool = True) -> List[Tuple[str, LightningWork]]:
        """Return its :class:`~lightning.lightning_app.core.work.LightningWork` with their names."""
    ...
    def set_state(self, provided_state: Dict, recurse: bool = True) -> None:
        ...
        :class:`~lightning.lightning_app.core.work.LightningWork`.
    ...
    def schedule(
        ...
        .. code-block:: python
            ...
            from lightning.lightning_app import LightningFlow
            from lightning.lightning_app.structures import List
    ...
    def configure_layout(self) -> Union[Dict[str, Any], List[Dict[str, Any]], Frontend]:
        ...
        1.  Return a single :class:`~lightning.lightning_app.frontend.frontend.Frontend` object to serve a user interface
            for this Flow.
        ...
        **Example:** Serve a static directory (with at least a file index.html inside).
        ...
        .. code-block:: python
            ...
            from lightning.lightning_app.frontend import StaticWebFrontend
        ...
        **Example:** Arrange the UI of my children in tabs (default UI by Lightning).
        ...
        Note:
            ...
            :class:`~lightning.lightning_app.frontend.frontend.Frontend`. These need to be provided at the time of app creation
    ...
    def configure_api(self):
        ...
        .. code-block:: python
            ...
            from lightning.lightning_app import LightningFlow
            from lightning.lightning_app.api import Post
