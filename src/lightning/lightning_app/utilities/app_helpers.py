...
from lightning.lightning_app.utilities.exceptions import LightningAppStateException
from lightning.lightning_app.utilities.tree import breadth_first

if TYPE_CHECKING:
    from lightning.lightning_app.core.app import LightningApp
    from lightning.lightning_app.core.flow import LightningFlow
    from lightning.lightning_app.utilities.types import Component
...
class InMemoryStateStore(StateStore):
    ...
    def set_app_state(self, k, v):
        ...
        if state_size > lightning.lightning_app.core.constants.APP_STATE_MAX_SIZE_BYTES:
            raise LightningAppStateException(
                f"App state size is {state_size} bytes, which is larger than the recommended size "
                f"of {lightning.lightning_app.core.constants.APP_STATE_MAX_SIZE_BYTES}. Please investigate this."
            )
...
def target_fn():
    ...
    async def update_fn():
        ...
        url = (
            ...
            else f"localhost:{lightning.lightning_app.core.constants.APP_SERVER_PORT}"
        )
...
def is_overridden(method_name: str, instance: Optional[object] = None, parent: Optional[Type[object]] = None) -> bool:
    if instance is None:
        return False
    if parent is None:
        if isinstance(instance, lightning.lightning_app.LightningFlow):
            parent = lightning.lightning_app.LightningFlow
        elif isinstance(instance, lightning.lightning_app.LightningWork):
            parent = lightning.lightning_app.LightningWork
        if parent is None:
...
def _is_json_serializable(x: Any) -> bool:
    ...
    if type(x) in lightning.lightning_app.core.constants.SUPPORTED_PRIMITIVE_TYPES:
...
def _set_child_name(component: "Component", child: "Component", new_name: str) -> str:
    ...
    if isinstance(child, lightning.lightning_app.core.LightningFlow):
    ...
    if isinstance(child, lightning.lightning_app.structures.Dict):
        for n, c in child.items():
            _set_child_name(child, c, n)
    if isinstance(child, lightning.lightning_app.structures.List):
        for c in child:
    ...
    for changed in delta_dict.values():
        for delta_key in changed.copy().keys():
            ...
            for p, c in _walk_to_component(root, component):
                ...
                if isinstance(c, lightning.lightning_app.core.LightningWork):
                    new_prefix += "['works']"

                if isinstance(c, lightning.lightning_app.core.LightningFlow):
                    new_prefix += "['flows']"

                if isinstance(c, (lightning.lightning_app.structures.Dict, lightning.lightning_app.structures.List)):
...
def _print_to_logger_info(*args, **kwargs):
    ...
    lightning.lightning_app._logger.info(" ".join([str(v) for v in args]))
...
def _is_headless(app: "LightningApp") -> bool:
    ...
    if app.frontends:
        return False
    for component in breadth_first(app.root, types=(lightning.lightning_app.LightningFlow,)):
