...
import lightning.lightning_fabric as lf
import lightning.pytorch_lightning as pl
from lightning.lightning_fabric.utilities.apply_func import convert_to_tensors
from lightning.lightning_fabric.utilities.cloud_io import get_filesystem
from lightning.lightning_fabric.utilities.device_dtype_mixin import _DeviceDtypeModuleMixin
from lightning.lightning_fabric.utilities.distributed import _distributed_available, _sync_ddp
from lightning.lightning_fabric.utilities.imports import _IS_WINDOWS, _TORCH_GREATER_EQUAL_1_11,_TORCH_GREATER_EQUAL_1_12
from lightning.lightning_fabric.utilities.types import Steppable
from lightning.lightning_fabric.wrappers import _FabricOptimizer
from lightning.pytorch_lightning.callbacks.callback import Callback
from lightning.pytorch_lightning.core.hooks import CheckpointHooks, DataHooks, ModelHooks
from lightning.pytorch_lightning.core.mixins import HyperparametersMixin
from lightning.pytorch_lightning.core.optimizer import LightningOptimizer
from lightning.pytorch_lightning.core.saving import ModelIO
from lightning.pytorch_lightning.loggers import Logger
from lightning.pytorch_lightning.trainer.connectors.logger_connector.fx_validator import _FxValidator
from lightning.pytorch_lightning.utilities import GradClipAlgorithmType
from lightning.pytorch_lightning.utilities.exceptions import MisconfigurationException
from lightning.pytorch_lightning.utilities.imports import _TORCH_GREATER_EQUAL_1_13
from lightning.pytorch_lightning.utilities.rank_zero import rank_zero_debug, rank_zero_warn, WarningCache
from lightning.pytorch_lightning.utilities.signature_utils import is_param_in_hook_signature
...
class LightningModule(
    ...
    def optimizers(self, use_pl_optimizer: bool = True) -> MODULE_OPTIMIZERS:
        ...
        Args:
            use_pl_optimizer: If ``True``, will wrap the optimizer(s) in a
                :class:`~lightning.pytorch_lightning.core.optimizer.LightningOptimizer` for automatic handling of precision and
                profiling.
    ...
    def training_step(self, *args: Any, **kwargs: Any) -> STEP_OUTPUT:  # type: ignore[return-value]
        ...
        Args:
            ...
            hiddens (``Any``): Passed in if
                :paramref:`~lightning.pytorch_lightning.core.module.LightningModule.truncated_bptt_steps` > 0.
    ...
    def predict_step(self, batch: Any, batch_idx: int, dataloader_idx: int = 0) -> Any:
        """Step function called during :meth:`~lightning.pytorch_lightning.trainer.trainer.Trainer.predict`. By default, it
        calls :meth:`~lightning.pytorch_lightning.core.module.LightningModule.forward`. Override to add any processing logic.

        The :meth:`~lightning.pytorch_lightning.core.module.LightningModule.predict_step` is used
        to scale inference on multi-devices.

        To prevent an OOM error, it is possible to use :class:`~lightning.pytorch_lightning.callbacks.BasePredictionWriter`
        callback to write the predictions to disk or database after each batch or on epoch end.

        The :class:`~lightning.pytorch_lightning.callbacks.BasePredictionWriter` should be used while using a spawn
    ...
    def configure_callbacks(self) -> Union[Sequence[Callback], Callback]:
        ...
        Lightning will make sure :class:`~lightning.pytorch_lightning.callbacks.model_checkpoint.ModelCheckpoint` callbacks
        run last.
    ...
    def configure_optimizers(self) -> Any:
        ...
        ``self.log('metric_to_track', metric_val)`` in your :class:`~lightning.pytorch_lightning.core.module.LightningModule`.
    ...
    def lr_scheduler_step(
        ...
        :class:`~lightning.pytorch_lightning.trainer.trainer.Trainer` calls each scheduler.
    ...
    def optimizer_step(
        ...
        r"""
        Override this method to adjust the default way the :class:`~lightning.pytorch_lightning.trainer.trainer.Trainer` calls
        each optimizer.
    ...
    def tbptt_split_batch(self, batch: Any, split_size: int) -> List[Any]:
        ...
        Note:
            ...
            :meth:`~lightning.pytorch_lightning.callbacks.base.Callback.on_train_batch_start`
            if :paramref:`~lightning.pytorch_lightning.core.module.LightningModule.truncated_bptt_steps` > 0.
        ...
        **kwargs: Any,
        ...
        Note:
            - Requires the implementation of the
              :meth:`~lightning.pytorch_lightning.core.module.LightningModule.forward` method.
