...
from lightning.lightning_fabric.utilities.cloud_io import get_filesystem
from lightning.lightning_fabric.utilities.data import _auto_add_worker_init_fn
from lightning.lightning_fabric.utilities.types import _PATH
from lightning.lightning_fabric.utilities.warnings import PossibleUserWarning
from lightning.pytorch_lightning.accelerators import Accelerator, TPUAccelerator
from lightning.pytorch_lightning.accelerators.xpu import XPUAccelerator
from lightning.pytorch_lightning.callbacks import Callback, Checkpoint, EarlyStopping, ProgressBarBase
from lightning.pytorch_lightning.callbacks.prediction_writer import BasePredictionWriter
from lightning.pytorch_lightning.core.datamodule import LightningDataModule
from lightning.pytorch_lightning.loggers import Logger
from lightning.pytorch_lightning.loggers.tensorboard import TensorBoardLogger
from lightning.pytorch_lightning.loops import PredictionLoop, TrainingEpochLoop
from lightning.pytorch_lightning.loops.dataloader.evaluation_loop import EvaluationLoop
from lightning.pytorch_lightning.loops.fit_loop import FitLoop
from lightning.pytorch_lightning.loops.utilities import _parse_loop_limits, _reset_progress
from lightning.pytorch_lightning.plugins import ApexMixedPrecisionPlugin, MixedPrecisionPlugin, PLUGIN_INPUT, PrecisionPlugin
from lightning.pytorch_lightning.profilers import Profiler
from lightning.pytorch_lightning.strategies import (
...
from lightning.pytorch_lightning.trainer import call, setup
from lightning.pytorch_lightning.trainer.configuration_validator import verify_loop_configurations
from lightning.pytorch_lightning.trainer.connectors.accelerator_connector import _LITERAL_WARN, AcceleratorConnector
from lightning.pytorch_lightning.trainer.connectors.callback_connector import CallbackConnector
from lightning.pytorch_lightning.trainer.connectors.checkpoint_connector import CheckpointConnector
from lightning.pytorch_lightning.trainer.connectors.data_connector import DataConnector
from lightning.pytorch_lightning.trainer.connectors.logger_connector import LoggerConnector
from lightning.pytorch_lightning.trainer.connectors.logger_connector.result import _OUT_DICT, _PBAR_DICT, _ResultCollection
from lightning.pytorch_lightning.trainer.connectors.signal_connector import SignalConnector
from lightning.pytorch_lightning.trainer.states import RunningStage, TrainerFn, TrainerState, TrainerStatus
from lightning.pytorch_lightning.trainer.supporters import CombinedLoader
from lightning.pytorch_lightning.tuner.tuning import _TunerResult, Tuner
from lightning.pytorch_lightning.utilities import GradClipAlgorithmType, parsing
from lightning.pytorch_lightning.utilities.argparse import (
...
from lightning.pytorch_lightning.utilities.auto_restart import _add_capture_metadata_collate
from lightning.pytorch_lightning.utilities.data import has_len_all_ranks
from lightning.pytorch_lightning.utilities.exceptions import ExitGracefullyException, MisconfigurationException
from lightning.pytorch_lightning.utilities.imports import _fault_tolerant_training
from lightning.pytorch_lightning.utilities.model_helpers import is_overridden
from lightning.pytorch_lightning.utilities.rank_zero import rank_zero_deprecation, rank_zero_info, rank_zero_warn
from lightning.pytorch_lightning.utilities.seed import isolate_rng
from lightning.pytorch_lightning.utilities.types import (
    _EVALUATE_OUTPUT,
    _PREDICT_OUTPUT,
    EVAL_DATALOADERS,
    LRSchedulerConfig,
...
class Trainer:
    ...
    def __init__(
        ...
        xpus: Optional[Union[List[int], str, int]] = None,
        ...
        Args:
            ...
            auto_select_gpus: If enabled and ``gpus`` or ``devices`` is an integer, pick available
                ...
                .. deprecated:: v1.9
                    ...
                    Please use the function :func:`~lightning.lightning_fabric.accelerators.cuda.find_usable_cuda_devices`
                    instead.

            benchmark: The value (``True`` or ``False``) to set ``torch.backends.cudnn.benchmark`` to.
                The value for ``torch.backends.cudnn.benchmark`` set in the current session will be used
                (``False`` if not manually set). If :paramref:`~lightning.pytorch_lightning.trainer.Trainer.deterministic` is set
            ...
            enable_checkpointing: If ``True``, enable checkpointing.
                ...
                :paramref:`~lightning.pytorch_lightning.trainer.trainer.Trainer.callbacks`.
                Default: ``True``.
    ...
    def fit(
        ...
        Args:
            ...
            train_dataloaders: A collection of :class:`torch.utils.data.DataLoader` or a
                :class:`~lightning.pytorch_lightning.core.datamodule.LightningDataModule` specifying training samples.
            ...
            datamodule: An instance of :class:`~lightning.pytorch_lightning.core.datamodule.LightningDataModule`.
        """
    ...
    def validate(
        ...
        Args:
            ...
            dataloaders: A :class:`torch.utils.data.DataLoader` or a sequence of them,
                or a :class:`~lightning.pytorch_lightning.core.datamodule.LightningDataModule` specifying validation samples.
        ...
        Returns:
            ...
            like :meth:`~lightning.pytorch_lightning.core.module.LightningModule.validation_step`,
            :meth:`~lightning.pytorch_lightning.core.module.LightningModule.validation_epoch_end`, etc.
    ...
    def test(
        ...
        Args:
            ...
            dataloaders: A :class:`torch.utils.data.DataLoader` or a sequence of them,
                or a :class:`~lightning.pytorch_lightning.core.datamodule.LightningDataModule` specifying test samples.
        ...
        Returns:
            ...
            like :meth:`~lightning.pytorch_lightning.core.module.LightningModule.test_step`,
            :meth:`~lightning.pytorch_lightning.core.module.LightningModule.test_epoch_end`, etc.
    ...
    def predict(
        ...
        Args:
            ...
            dataloaders: A :class:`torch.utils.data.DataLoader` or a sequence of them,
                or a :class:`~lightning.pytorch_lightning.core.datamodule.LightningDataModule` specifying prediction samples.
    ...
    def tune(
        ...
        Args:
            ...
            scale_batch_size_kwargs: Arguments for :func:`~lightning.pytorch_lightning.tuner.batch_size_scaling.scale_batch_size`

            lr_find_kwargs: Arguments for :func:`~lightning.pytorch_lightning.tuner.lr_finder.lr_find`
    ...
    def model(self) -> Optional[torch.nn.Module]:
        ...
        :meth:`~lightning.pytorch_lightning.trainer.trainer.Trainer.lightning_module` instead.
        """
    ...
    def early_stopping_callback(self) -> Optional[EarlyStopping]:
        """The first :class:`~lightning.pytorch_lightning.callbacks.early_stopping.EarlyStopping` callback in the
    ...
    def early_stopping_callbacks(self) -> List[EarlyStopping]:
        """A list of all instances of :class:`~lightning.pytorch_lightning.callbacks.early_stopping.EarlyStopping` found in
        the Trainer.callbacks list."""
        return [c for c in self.callbacks if isinstance(c, EarlyStopping)]

    @property
    def prediction_writer_callbacks(self) -> List[BasePredictionWriter]:
        """A list of all instances of :class:`~lightning.pytorch_lightning.callbacks.prediction_writer.BasePredictionWriter`
    ...
    def checkpoint_callback(self) -> Optional[Checkpoint]:
        """The first :class:`~lightning.pytorch_lightning.callbacks.model_checkpoint.ModelCheckpoint` callback in the
        Trainer.callbacks list, or ``None`` if it doesn't exist."""
    ...
    def checkpoint_callbacks(self) -> List[Checkpoint]:
        """A list of all instances of :class:`~lightning.pytorch_lightning.callbacks.model_checkpoint.ModelCheckpoint` found
        in the Trainer.callbacks list."""
        return [c for c in self.callbacks if isinstance(c, Checkpoint)]

    @property
    def progress_bar_callback(self) -> Optional[ProgressBarBase]:
        """An instance of :class:`~lightning.pytorch_lightning.callbacks.progress.base.ProgressBarBase` found in the
    ...
    def ckpt_path(self) -> Optional[str]:
        """Set to the path/URL of a checkpoint loaded via :meth:`~lightning.pytorch_lightning.trainer.trainer.Trainer.fit`,
        :meth:`~lightning.pytorch_lightning.trainer.trainer.Trainer.validate`,
        :meth:`~lightning.pytorch_lightning.trainer.trainer.Trainer.test`, or
        :meth:`~lightning.pytorch_lightning.trainer.trainer.Trainer.predict`. ``None`` otherwise."""
    ...
    def fit_loop(self, loop: FitLoop) -> None:
        ...
        It will run with
        :meth:`~lightning.pytorch_lightning.trainer.trainer.Trainer.fit`.
        """
        loop.trainer = self
        self._fit_loop = loop
    ...
    def validate_loop(self, loop: EvaluationLoop) -> None:
        ...
        It will run with
        :meth:`~lightning.pytorch_lightning.trainer.trainer.Trainer.validate`. Note that this loop is different from the one
        running during training inside the :meth:`lightning.pytorch_lightning.trainer.trainer.Trainer.fit` call.
        """
        loop.trainer = self
    ...
    def test_loop(self, loop: EvaluationLoop) -> None:
        ...
        It will run with
        :meth:`~lightning.pytorch_lightning.trainer.trainer.Trainer.test`.
        """
        loop.trainer = self
    ...
    def predict_loop(self, loop: PredictionLoop) -> None:
        ...
        It will run with
        :meth:`~lightning.pytorch_lightning.trainer.trainer.Trainer.predict`.
        """
        loop.trainer = self
    ...
    def callback_metrics(self) -> Dict:
        ...
        # `lightning.pytorch_lightning.trainer.connectors.logger_connector.result._OUT_DICT`
