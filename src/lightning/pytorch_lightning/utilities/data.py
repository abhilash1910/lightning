...
from lightning.lightning_fabric.utilities.data import _reinstantiate_wrapped_cls, _replace_value_in_saved_args
from lightning.lightning_fabric.utilities.data import has_iterable_dataset as new_has_iterable_dataset
from lightning.lightning_fabric.utilities.data import has_len as new_has_len
from lightning.pytorch_lightning.overrides.distributed import IndexBatchSamplerWrapper
from lightning.pytorch_lightning.trainer.states import RunningStage
from lightning.pytorch_lightning.trainer.supporters import CombinedLoader
from lightning.pytorch_lightning.utilities.auto_restart import CaptureIterableDataset, CaptureMapDataset, FastForwardSampler
from lightning.pytorch_lightning.utilities.enums import _FaultTolerantMode
from lightning.pytorch_lightning.utilities.exceptions import MisconfigurationException
from lightning.pytorch_lightning.utilities.rank_zero import rank_zero_deprecation, rank_zero_warn, WarningCache
...
def has_len_all_ranks(
    ...
    # we are checking using lightning.lightning_fabric, which doesn't know CombinedLoader
...
def has_iterable_dataset(*args: Any, **kwargs: Any) -> Any:
    rank_zero_deprecation(
        "`lightning.pytorch_lightning.utilities.data.has_iterable_dataset` has been deprecated in v1.8.0 and will be"
        " removed in v1.10.0. Please use `lightning.lightning_fabric.utilities.data.has_iterable_dataset` instead."
    )
...
def has_len(*args: Any, **kwargs: Any) -> Any:
    rank_zero_deprecation(
        "`lightning.pytorch_lightning.utilities.data.has_len` has been deprecated in v1.8.0 and will be"
        " removed in v1.10.0. Please use `lightning.lightning_fabric.utilities.data.has_len` instead."
    )
