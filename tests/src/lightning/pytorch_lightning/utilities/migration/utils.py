...
from lightning.lightning_fabric.utilities.imports import _IS_WINDOWS
...
from lightning.pytorch_lightning.utilities.migration.migration import _migration_index
...
class pl_legacy_patch:
    ...
    unpickling old checkpoints. The following patches apply.
        ...
        1. ``lightning.pytorch_lightning.utilities.argparse._gpus_arg_default``: Applies to all checkpoints saved prior to
           version 1.2.8. See: https://github.com/PyTorchLightning/pytorch-lightning/pull/6898
        2. ``lightning.pytorch_lightning.utilities.argparse_utils``: A module that was deprecated in 1.2 and removed in 1.4,
    ...
    def __enter__(self) -> "pl_legacy_patch":
        ...
        legacy_argparse_module = ModuleType("lightning.pytorch_lightning.utilities.argparse_utils")
        sys.modules["lightning.pytorch_lightning.utilities.argparse_utils"] = legacy_argparse_module
    ...
    def __exit__(
        ...
        del sys.modules["lightning.pytorch_lightning.utilities.argparse_utils"]
...
def _pl_migrate_checkpoint(checkpoint: _CHECKPOINT, checkpoint_path: Optional[_PATH] = None) -> _CHECKPOINT:
    ...
    _log.info(
        ...
        f" `python -m lightning.pytorch_lightning.utilities.upgrade_checkpoint --file {str(path_hint)}`"
    )
    return checkpoint
