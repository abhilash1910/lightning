...
from lightning.lightning_fabric.utilities.logger import _add_prefix, _convert_params, _flatten_dict
...
class MLFlowLogger(Logger):
    ...
    .. code-block:: python
        ...
        from lightning.pytorch_lightning.loggers import MLFlowLogger

        mlf_logger = MLFlowLogger(experiment_name="lightning_logs", tracking_uri="file:./ml-runs")
        trainer = Trainer(logger=mlf_logger)

    Use the logger anywhere in your :class:`~lightning.pytorch_lightning.core.module.LightningModule` as follows:
    ...
    Args:
        ...
        log_model: Log checkpoints created by :class:`~lightning.pytorch_lightning.callbacks.model_checkpoint.ModelCheckpoint`
            as MLFlow artifacts.

            * if ``log_model == 'all'``, checkpoints are logged during training.
            * if ``log_model == True``, checkpoints are logged at the end of training, except when
              :paramref:`~lightning.pytorch_lightning.callbacks.Checkpoint.save_top_k` ``== -1``
