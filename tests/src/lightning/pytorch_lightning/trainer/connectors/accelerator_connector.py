...
from lightning.lightning_fabric.plugins.environments import (
    ClusterEnvironment,
    KubeflowEnvironment,
...
from lightning.lightning_fabric.utilities import _StrategyType
from lightning.lightning_fabric.utilities.device_parser import _determine_root_gpu_device, _determine_root_xpu_device
from lightning.lightning_fabric.utilities.imports import _IS_INTERACTIVE, _TORCH_GREATER_EQUAL_1_11
from lightning.pytorch_lightning.accelerators import AcceleratorRegistry
from lightning.pytorch_lightning.accelerators.accelerator import Accelerator
from lightning.pytorch_lightning.accelerators.cpu import CPUAccelerator
from lightning.pytorch_lightning.accelerators.cuda import CUDAAccelerator
from lightning.pytorch_lightning.accelerators.hpu import HPUAccelerator
from lightning.pytorch_lightning.accelerators.ipu import IPUAccelerator
from lightning.pytorch_lightning.accelerators.mps import MPSAccelerator
from lightning.pytorch_lightning.accelerators.tpu import TPUAccelerator
from lightning.pytorch_lightning.accelerators.xpu import XPUAccelerator
from lightning.pytorch_lightning.plugins import (
...
from lightning.pytorch_lightning.plugins import XPUPrecisionPlugin,XPUBf16PrecisionPlugin
from lightning.pytorch_lightning.plugins.environments import BaguaEnvironment
...
from lightning.pytorch_lightning.strategies.ddp_spawn import _DDP_FORK_ALIASES
from lightning.pytorch_lightning.strategies.horovod import _HOROVOD_AVAILABLE
from lightning.pytorch_lightning.tuner.auto_gpu_select import pick_multiple_gpus
from lightning.pytorch_lightning.tuner.auto_xpu_select import pick_multiple_xpus
from lightning.pytorch_lightning.utilities.exceptions import MisconfigurationException
from lightning.pytorch_lightning.utilities.imports import _IPU_AVAILABLE
...
class AcceleratorConnector:
    def __init__(
        ...
        auto_select_xpus: Optional[bool] = None,
        ...
        self._auto_select_xpus: Optional[bool] = auto_select_xpus
        ...
        self._check_device_config_and_set_final_flags(
            devices=devices, num_nodes=num_nodes, num_processes=num_processes, gpus=gpus, xpus=xpus, ipus=ipus, tpu_cores=tpu_cores
        )
        ...
        if self._accelerator_flag == "xpu":
            self._accelerator_flag=self._choose_xpu_accelerator_backend()
        elif self._accelerator_flag == "auto" or self._accelerator_flag is None:
    ...
    def _check_config_and_set_final_flags(
        ...
        if (
            ...
            and accelerator not in ("auto", "gpu", "xpu")
    ...
    def _check_device_config_and_set_final_flags(
        ...
        xpus: Optional[Union[List[int], str, int]],
        ipus: Optional[int],
        ...
        self._map_deprecated_devices_specific_info_to_accelerator_and_device_flag(
            devices, num_processes, gpus, xpus, ipus, tpu_cores
        )

        if self._devices_flag == "auto" and self._accelerator_flag is None:
            raise MisconfigurationException(
                f"You passed `devices={devices}` but haven't specified"
                " `accelerator=('auto'|'tpu'|'gpu'|'ipu'|'cpu'|'hpu'|'mps'|'xpu')` for the devices mapping."
            )
    ...
    def _map_deprecated_devices_specific_info_to_accelerator_and_device_flag(
        ...
        if xpus is not None:
            rank_zero_deprecation(
                f"Setting `Trainer(xpus={gpus!r})` is deprecated in v1.7 and will be removed"
                f" in v2.0. Please use `Trainer(accelerator='xpu', devices={xpus!r})` instead."
            )
        if gpus is not None:
            rank_zero_deprecation(
        ...
        self._xpus: Optional[Union[List[int], str, int]] = xpus
        self._tpu_cores: Optional[Union[List[int], str, int]] = tpu_cores
        deprecated_devices_specific_flag = num_processes or gpus or ipus or tpu_cores or xpus
        if deprecated_devices_specific_flag and deprecated_devices_specific_flag not in ([], 0, "0"):
            ...
            if [(num_processes is not None), (gpus is not None), (xpus is not None), (ipus is not None), (tpu_cores is not None)].count(
                True
            ) > 1:
                # TODO: improve error message
    ...
    @staticmethod
    def _choose_xpu_accelerator_backend() -> str:
        if XPUAccelerator.is_available():
            return "xpu"
        
        raise MisconfigurationException("No supported xpu backend found!")
    ...
    def _set_parallel_devices_and_init_accelerator(self) -> None:
        ...
        self._xpus = self._devices_flag if not self._xpus else self._xpus 
        ...
        self._set_devices_flag_if_auto_select_xpus_passed()
    ...
    def _set_devices_flag_if_auto_select_xpus_passed(self) -> None:
        if self._auto_select_xpus is not None:
            rank_zero_deprecation(
                "The Trainer argument `auto_select_xpus` has been deprecated in v1.9.0 and will be removed in v1.10.0."
                " Please use the function `lightning.pytorch_lightning.accelerators.find_usable_xpu_devices` instead."
            )
        if self._auto_select_xpus and isinstance(self._xpus, int) and isinstance(self.accelerator, XPUAccelerator):
            self._devices_flag = pick_multiple_xpus(
                self._xpus,
                # we already show a deprecation message when user sets Trainer(auto_select_xpus=...)
                _show_deprecation=False,
            )
            log.info(f"Auto select gpus: {self._devices_flag}")
    ...
    def _choose_strategy(self) -> Union[Strategy, str]:
        ...
        if len(self._parallel_devices) <= 1:
            ...
            elif isinstance(self._accelerator_flag, XPUAccelerator) or (
                isinstance(self._accelerator_flag, str) and self._accelerator_flag in ("xpu")
            ):
                device = _determine_root_xpu_device(self._parallel_devices)
            else:
                device = "cpu"
    ...
    def _check_strategy_and_fallback(self) -> None:
        ...
        if (
            ...
            raise MisconfigurationException(
                ...
                "but GPU|XPU accelerator is not used."
            )
    ...
    def _check_and_init_precision(self) -> PrecisionPlugin:
        ...
        ###to-do bf16 xpu amp
        if isinstance(self.accelerator, XPUAccelerator):
            if self._precision_flag in (16, "bf16"):
                if self._precision_flag == 16:
                    rank_zero_warn(
                        "You passed `Trainer(accelerator='xpu', precision=16)` but AMP"
                        " is not supported with XPUs yet. Using `precision='bf16'` instead."
                    )
                    return XPUPrecisionPlugin()
                else:
                    return XPUBf16PrecisionPlugin()
            else:
                return XPUPrecisionPlugin()
