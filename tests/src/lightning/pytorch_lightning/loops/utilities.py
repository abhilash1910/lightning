...
from lightning.pytorch_lightning.strategies.strategy import Strategy
from lightning.pytorch_lightning.trainer.progress import BaseProgress
...
def _extract_hiddens(training_step_output: STEP_OUTPUT, truncated_bptt_steps: int) -> Optional[Any]:
    ...
    Raises:
        MisconfigurationException: If :attr:`~lightning.pytorch_lightning.core.Lightning.LightningModule.truncated_bptt_steps` is
...
def _block_parallel_sync_behavior(strategy: Strategy, block: bool = True) -> Generator[None, None, None]:
    """Blocks synchronization in :class:`~lightning.pytorch_lightning.strategies.parallel.ParallelStrategy`. This is useful
