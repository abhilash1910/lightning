...
import lightning.lightning_fabric.accelerators as accelerators  # avoid circular dependency
from lightning.lightning_fabric.plugins.environments.torchelastic import TorchElasticEnvironment
from lightning.lightning_fabric.utilities.exceptions import MisconfigurationException
from lightning.lightning_fabric.utilities.types import _DEVICE
...
def _parse_gpu_ids(
    ...
    :class:`~lightning.pytorch_lightning.trainer.Trainer`.
...
def _determine_root_xpu_device(xpus: List[_DEVICE]) -> Optional[_DEVICE]:
    """
    Args:
        gpus: Non-empty list of ints representing which GPUs to use
    ...
    if not isinstance(xpus, list):
        raise TypeError("XPUs should be a list")

    assert len(xpus) > 0, "GPUs should be a non-empty list"
...
def _parse_xpu_ids(
    xpus: Optional[Union[int, str, List[int]]],
    include_cuda: bool = False,
    include_mps: bool = False,
) -> Optional[List[int]]:
    """
    ...
    # Handle the case when no GPUs are requested
    if xpus is None or (isinstance(xpus, int) and xpus == 0) or str(xpus).strip() in ("0", "[]"):
        return None

    # We know the user requested GPUs therefore if some of the
    # requested GPUs are not available an exception is thrown.
    xpus = _normalize_parse_xpu_string_input(xpus)
    xpus = _normalize_parse_xpu_input_to_list(xpus, include_cuda=include_cuda, include_mps=include_mps)
    if not xpus:
    ...
    if (
        TorchElasticEnvironment.detect()
        and len(xpus) != 1
        and len(_get_all_available_xpus(include_cuda=include_cuda, include_mps=include_mps)) == 1
    ):
        # Omit sanity check on torchelastic because by default it shows one visible GPU per process
        return xpus
    ...
    return _sanitize_xpu_ids(xpus, include_cuda=include_cuda, include_mps=include_mps)


def _normalize_parse_xpu_string_input(s: Union[int, str, List[int]]) -> Union[int, List[int]]:
    if not isinstance(s, str):
        return s
    if s == "-1":
        return -1
...
def _sanitize_xpu_ids(xpus: List[int], include_cuda: bool = False, include_mps: bool = False) -> List[int]:
    """Checks that each of the GPUs in the list is actually available. Raises a MisconfigurationException if any of
    the GPUs is not available.
    ...
    #if sum((include_cuda, include_mps)) == 0:
    #    raise ValueError("At least one gpu type should be specified!")
    all_available_xpus = _get_all_available_xpus(include_cuda=include_cuda, include_mps=include_mps)
    for xpu in xpus:
        if xpu not in all_available_xpus:
            raise MisconfigurationException(
                f"You requested xpu: {xpus}\n But your machine only has: {all_available_xpus}"
            )
    return xpus


def _normalize_parse_xpu_input_to_list(
    xpus: Union[int, List[int], Tuple[int, ...]], include_cuda: bool, include_mps: bool
) -> Optional[List[int]]:
    assert xpus is not None
    ...
    if not xpus:  # xpus==0
        return None
    if xpus == -1:
        return _get_all_available_xpus(include_cuda=include_cuda, include_mps=include_mps)

    return list(range(xpus))


def _get_all_available_xpus(include_cuda: bool = False, include_mps: bool = False) -> List[int]:
    """
    Returns:
        A list of all available GPUs
    """
    #cuda_gpus = accelerators.cuda._get_all_visible_cuda_devices() if include_cuda else []
    #mps_gpus = accelerators.mps._get_all_available_mps_gpus() if include_mps else []
    xpus=accelerators.xpu._get_all_visible_xpu_devices()
    return xpus
...
def _check_unique(device_ids: List[int]) -> None:
    ...
    """
    if len(device_ids) != len(set(device_ids)):
        raise MisconfigurationException("Device ID's (XPU) must be unique.")
...
def _check_data_type(device_ids: Any) -> None:
    ...
    Raises:
        MisconfigurationException:
            If ``device_ids`` of GPU/TPUs aren't ``int``, ``str``, sequence of ``int`` or ``None``
    """
    msg = "Device IDs (XPU) must be an int, a string, a sequence of ints or None, but you passed"
