...
#from lightning.lightning_fabric.accelerators.registry import AcceleratorRegistry
from lightning.lightning_fabric.utilities.imports import (
...
class XPUAccelerator(Accelerator):
    ...
    def parse_devices(devices: Union[int, str, List[int]]) -> Optional[List[int]]:
        ...
        from lightning.lightning_fabric.utilities.device_parser import _parse_xpu_ids
...
#AcceleratorRegistry.register("xpu", XPUAccelerator, "Custom xpu accelerator")
...
def find_usable_xpu_devices(num_devices: int = -1) -> List[int]:
    ...
    for xpu_idx in visible_devices:
        try:
            torch.tensor(0, device=torch.xpu.device(xpu_idx))
        except RuntimeError:
            unavailable_devices.append(xpu_idx)
            continue

        available_devices.append(xpu_idx)
        if len(available_devices) == num_devices:
            # exit early if we found the right number of XPUs
            break
...
def _get_all_visible_xpu_devices() -> List[int]:
    """Returns a list of all visible XPU devices.
...
@contextmanager
def _patch_xpu_is_available() -> Generator:
    """Context manager that safely patches :func:`torch.xpu.is_available` with its NVML-based version if
    possible."""
    if hasattr(torch._C, "_cuda_getDeviceCount") and _device_count_nvml() >= 0 and not _TORCH_GREATER_EQUAL_2_0:
        ...
        orig_check =torch.xpu.is_available()
...
def is_xpu_available() -> bool:
    ...
    if the platform allows it.
    """
    # We set `PYTORCH_NVML_BASED_CUDA_CHECK=1` in lightning.lightning_fabric.__init__.py
    #return get_accelerator().is_available() #if _TORCH_GREATER_EQUAL_2_0 else num_cuda_devices() > 0
    return torch.xpu.is_available()

# TODO: Remove once minimum supported PyTorch version is 1.13
def _parse_visible_devices() -> Set[int]:
    """Implementation copied from upstream: https://github.com/pytorch/pytorch/pull/84879."""
    var = os.getenv("XPU_VISIBLE_DEVICES")
    if var is None:
