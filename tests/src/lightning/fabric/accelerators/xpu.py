...
#from lightning.fabric.accelerators.registry import AcceleratorRegistry
...
class XPUAccelerator(Accelerator):
    ...
    def setup_device(self, device: get_accelerator().device_name()) -> None:
        """
        Raises:
            ValueError:
...
def find_usable_xpu_devices(num_devices: int = -1) -> List[int]:
    ...
    for xpu_idx in visible_devices:
        try:
            torch.tensor(0, device=get_accelerator().device(xpu_idx))
        except RuntimeError:
...
def _patch_xpu_is_available() -> Generator:
    ...
    if hasattr(torch._C, "_cuda_getDeviceCount") and _device_count_nvml() >= 0 and not _TORCH_GREATER_EQUAL_2_0:
        ...
        orig_check = get_accelerator().is_available()
        torch.cuda.is_available = get_accelerator().is_available()
        try:
            yield
        finally:
