...
from lightning.lightning_app.core.queues import MultiProcessQueue
from lightning.lightning_app.storage import Path
from lightning.lightning_app.storage.copier import _Copier, _copy_files
from lightning.lightning_app.storage.drive import _maybe_create_drive, Drive
from lightning.lightning_app.storage.path import _path_to_work_artifact
from lightning.lightning_app.storage.payload import Payload
from lightning.lightning_app.utilities.app_helpers import affiliation
from lightning.lightning_app.utilities.component import _set_work_context
...
from lightning.lightning_app.utilities.exceptions import CacheMissException, LightningSigtermStateException

if TYPE_CHECKING:
    from lightning.lightning_app import LightningWork
...
class ProxyWorkRun:
        ...
        def warn_if_pathlike(obj: Union[os.PathLike, str]):
            ...
            if os.sep in str(obj) and os.path.exists(obj):
                ...
                warnings.warn(
                    ...
                    f" `lightning.lightning_app.storage.Path` object to be able to access these files in your Work.",
                    UserWarning,
                )
    ...
    def _convert_hashable(args: Tuple[Any, ...], kwargs: Dict[str, Any]) -> Tuple[Tuple[Any, ...], Dict[str, Any]]:
        ...
        Returns:
            The positional and keyword arguments in the same order they were passed in.
        """
        from lightning.lightning_app.utilities.types import Hashable
...
class WorkRunExecutor:
    ...
    def process_queue(queue):
        from lightning.lightning_app.core.queues import HTTPQueue, RedisQueue
...
class WorkRunner:
    ...
    def setup(self):
        from lightning.lightning_app.utilities.state import AppState
    ...
    def _process_call_args(
        ...
        This method currently only implements special treatments for the :class:`lightning.lightning_app.storage.path.Path`
...
def persist_artifacts(work: "LightningWork") -> None:
    """Copies all :class:`~lightning.lightning_app.storage.path.Path` referenced by the given LightningWork to the shared
    storage.
