...
from lightning.lightning_app.storage.payload import Payload
from lightning.lightning_app.utilities.app_helpers import _is_json_serializable, _LightningAppRef, is_overridden
from lightning.lightning_app.utilities.app_status import WorkStatus
from lightning.lightning_app.utilities.component import _is_flow_context, _sanitize_state
from lightning.lightning_app.utilities.enum import (
    CacheCallsKeys,
    make_status,
    WorkFailureReasons,
    WorkStageStatus,
...
from lightning.lightning_app.utilities.exceptions import LightningWorkException
from lightning.lightning_app.utilities.introspection import _is_init_context
from lightning.lightning_app.utilities.network import find_free_network_port
from lightning.lightning_app.utilities.packaging.build_config import BuildConfig
from lightning.lightning_app.utilities.packaging.cloud_compute import (
    _CLOUD_COMPUTE_STORE,
    _CloudComputeStore,
    _maybe_create_cloud_compute,
    CloudCompute,
)
from lightning.lightning_app.utilities.proxies import Action, LightningWorkSetAttrProxy, ProxyWorkRun, unwrap, WorkRunExecutor
...
class LightningWork:
    ...
    def __init__(
        ...
        The LightningApp runs its :class:`~lightning.lightning_app.core.flow.LightningFlow` component
        ...
        if self._is_state_attribute(name):
            if isinstance(value, (LightningFlow, LightningWork)):
                ...
                raise AttributeError(
                    ...
                    "objects in the state, you can use the `lightning.lightning_app.storage.Payload` API."
                )
