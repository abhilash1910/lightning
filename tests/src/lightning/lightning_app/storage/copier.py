...
from lightning.lightning_app.storage.path import _filesystem
from lightning.lightning_app.storage.requests import _ExistsRequest, _GetRequest
...
class _Copier(Thread):
    ...
    :class:`~lightning.lightning_app.storage.orchestrator.StorageOrchestrator`,
    ...
    :func:`~lightning.lightning_app.storage.path.shared_storage_path`). Any errors raised during the copy will be added to the
    ...
    def __init__(
        self, work: "lightning.lightning_app.LightningWork", copy_request_queue: "BaseQueue", copy_response_queue: "BaseQueue"
    ) -> None:
    ...
    def run_once(self):
        ...
        obj: Optional[lightning.lightning_app.storage.Path] = _find_matching_path(self._work, request)
        if obj is None:
            # If it's not a path, it must be a payload
            obj: lightning.lightning_app.storage.Payload = getattr(self._work, request.name)
...
def _find_matching_path(work, request: _GetRequest) -> Optional["lightning.lightning_app.storage.Path"]:
    for name in work._paths:
        candidate: lightning.lightning_app.storage.Path = getattr(work, name)
